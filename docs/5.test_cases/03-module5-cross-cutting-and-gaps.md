...
- 🟨 **Partially covered (was 🔴): a salary slip's LOP drifting from the leave record after the slip was issued.** Analysed as six scenarios — late approval, backdated submission, HR override in either direction, a second approval after the run, and flipping a leave type's `counts_as_lop` — of which **five overpay**, and an overpayment is the one nobody ever queries. Closed by prevention rather than re-sync, because a payslip *should* be a snapshot: rewriting one an employee has already received as a PDF would destroy the record of what was actually paid.
  ...
  - **The reconciliation check that was designed alongside these two is now built** — `voidInconsistentSlips` compares each `ACTIVE` slip's stored `lop_days` against a live `findLopWorkingDays` alongside the employed-day comparison it already made, and voids the slip when either disagrees. It arrived as part of the holiday-recount work rather than this gap, because a holiday declared inside someone's approved *unpaid* leave changes their LOP with no leave decision involved — exactly the shape scenario E has.
  - **Still open, and the reason this is 🟨 not ✅:** flipping `counts_as_lop` on a leave type changes historical LOP for **every** employee at once, and `setLeaveTypeStatus`/`updateLeaveType` don't call the check — so the detection exists but nothing triggers it for that one path. Wiring it is small (iterate the affected employees, call the same function) but it is a fan-out over the whole company rather than one person, so it wants its own decision about batching and notification volume, not a quiet addition to an unrelated change. Also still open: whether an `EXPIRED` status should replace M5's advisory notification, which is a product decision about what the employee sees rather than a technical one.
