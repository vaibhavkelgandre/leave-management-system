...
#### 📝 `leave_requests`
*Migrations: `013_create_leave_requests.sql`, `043_alter_leave_requests_add_hr_escalated.sql`*
...
| `hr_escalated` | `BOOLEAN` | `NOT NULL`, default `false` | *Migration 043.* True when the employee was actively standing in as **their own manager's** delegate at the moment they submitted — that manager is away, so HR decides instead (the only case where HR may approve/reject directly rather than override afterward). **Stored rather than derived on purpose:** the same question could be asked of `delegations` at decision time, but the answer flips the moment the window lapses, so an escalated request nobody got round to deciding would silently stop being HR's to decide. Written once at submit, never rewritten |
...
**Indexes:** `employee_id`, `(employee_id, status)` (overlap checks and "my requests" both filter this way), a partial `(employee_id, status) WHERE hr_escalated` (migration 043 — HR's pending-approvals badge counts escalated requests across a whole branch, and only the true rows are ever searched for), and `(employee_id, start_date DESC)` — added by migration 037 for HR's **paginated** browse view, whose query is always `WHERE employee_id = ANY(subtree) … ORDER BY start_date DESC LIMIT/OFFSET`. The first two indexes can find those rows but not return them ordered, so without the third every page sorts the whole matching set before discarding all but 25 rows — which would make page 1 no cheaper than the unpaginated query it replaced. `DESC` is part of the index, not decoration: an ASC index can only be read backwards for a single key, not for this multi-key `= ANY(...)` case.
...
| `reason` | `VARCHAR(30)` | `CHECK IN ('SUBMIT','APPROVE','REJECT','WITHDRAW','CANCEL','HR_OVERRIDE_APPROVE','HR_OVERRIDE_REJECT','HOLIDAY_ADJUSTMENT')` (042) | Descriptive only — not read by any query logic. `'HOLIDAY_ADJUSTMENT'` (042) is the only reason not tied to a state transition: a holiday declared or removed inside a live request recounts its working days, and the difference is corrected by **appending** an entry rather than editing the earlier ones, so the summed balance still agrees with the history that produced it (NFR-2). SUPER_ADMIN's auto-approve bypass reuses `'APPROVE'` (no new tag needed) but writes only that one entry, never a preceding `'SUBMIT'` — there's no pending state to represent when a request never passes through SUBMITTED |
