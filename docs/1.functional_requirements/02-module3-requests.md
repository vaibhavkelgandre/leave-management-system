...
*Implemented via `delegations` table / `delegationService.js` / `POST /api/delegations` (manager-only, `manager_id` always the caller — never client-supplied) / `PATCH /api/delegations/:id` / `GET /api/delegations/mine`. A delegate's authority is checked live, per request, via `delegationRepository.findActiveDelegation` (today's date within the delegation's range) — not cached or assumed. `audit_logs.acted_for` records the manager being represented whenever a delegate (not the manager themself) acts. Frontend: `DelegationsPage.jsx` / `DelegationForm.jsx`. Tested in `delegations.test.js` and `leaveRequests.test.js`, including the named case: a delegate's authority stops when their window ends (`404` outside it, `200` inside it).*

*A nomination can also be **edited** (`PATCH /api/delegations/:id`, `delegationEdit.test.js`), which is what makes rule 3 below actionable: the manager it notifies can now reassign instead of only being told. Owner-only (`404` otherwise), re-running every nomination guard against the merged result, refused once the window has ended. The outgoing delegate of a swap is told (`DELEGATION_REVOKED`); a delegate whose dates merely moved gets `DELEGATION_UPDATED`. There is still no revoke endpoint and still no accept/reject flow — neither is asked for here.*
...
3. Leave overlapping a window that has **not started** is allowed, and both sides get a `DELEGATION_LEAVE_CONFLICT` notification — refusing would let a nomination the delegate never agreed to veto their leave, and this FR has no accept/reject flow for them to decline through. The manager's half of that notification is acted on by editing the delegation (above).
