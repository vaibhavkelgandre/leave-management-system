...
> 🧾 **A re-run keeps the salary the period was originally run with.** `salary_structures` holds one row per employee, overwritten on every change and carrying no effective dates, so there is no way to ask "what was this person earning in July". A re-run therefore used to recompute at whatever the salary is *now* — and since void-and-re-run is how every correction happens (a late leave approval, a leaving date, a stale pro-ration), a correction made after a raise silently overpaid the corrected month. On a ₹50,000 employee raised to ₹60,000 that was ₹10,000 on a full month, and still ₹3,225.81 on a pro-rated exit month.
>
> So when a period already has a slip — `ACTIVE` **or** `VOIDED` — the salary components are taken from that slip rather than from the current structure. The slip already stores the whole basis (`basic_pay`, `hra`, both PF figures, `esic`, `special_allowance`, `income_tax`), so this needs no schema change. Rows computed this way carry **`usedArchivedSalary: true`**, and the run preview says so, because "I gave her a raise, re-ran July, and the number didn't move" otherwise reads as a bug.
>
> The rule is: **a correction recomputes days, never salary.** Two consequences worth knowing — a current structure is still required (its *existence* is what marks someone as on payroll, so removing it stops payment rather than letting an archive continue it), and a genuinely **backdated raise cannot be applied** to a period that has already been run. That would need `effective_from` on the structure itself, which is a migration and an HR field, and is only the right answer once backdated raises are a real workflow.
