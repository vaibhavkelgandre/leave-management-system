...
| **LOW** | No explicit JWT `algorithms` allowlist on verify | `jwt.verify(token, secret)` without pinning `algorithms:["HS256"]` — not exploitable today with a fixed symmetric secret, but missing defense-in-depth. Still open, though narrower than it reads: `authMe.test.js` confirms jsonwebtoken already refuses an `alg: none` token when a secret is supplied, so the classic downgrade is closed by the library rather than by us. Pinning it is still worth doing (recommendation 3). |
