...
> - **On update, only when the dates actually change.** `updateHolidaySchema` *is* `createHolidaySchema` — a holiday has no partial edit, so the client resends the whole record — and checking unconditionally would make a legacy holiday impossible to *rename*, which the rule is not about.
> - **Leave submission was deliberately left open** in the same pass, on direct decision. There is no "HR raises leave on behalf of an employee" endpoint, so blocking past start dates would make a backdated absence unrecordable by anyone and its LOP undeductible; and backdated submission is already an analysed, handled scenario of the payroll drift design (G21), which refuses the *decision* via `assertPeriodsOpen` rather than the submission. Don't add that block without also adding the on-behalf-of path.
>
...
> 🧾 **A migration that re-declares a CHECK list must copy the *newest* list, and there is now a test that proves it did.** There is no `ADD VALUE` for a CHECK constraint, so every migration adding a `notifications.type` does `DROP CONSTRAINT` then `ADD CONSTRAINT` with the whole list re-typed — which means copying an older ancestor's list silently *deletes* every value added in between. Hit for real writing `044`: its list came from `041` and dropped `LEAVE_DAYS_ADJUSTED`, which `042` had added. Three things worth keeping:
> - **The failure is invisible without a test.** Every `notify*` helper catches and logs its own error and never rethrows (deliberately — a notification must not fail the real action), so a type the constraint no longer accepts just stops producing notifications. The triggering action still succeeds and still answers `200`. What surfaced it was an unrelated assertion in `holidayAdjustment.test.js` counting rows, not anything pointing at the constraint.
> - **`notificationTypeConstraint.test.js` is the guard**: it greps the `type: "X"` literals out of `notificationService.js` and inserts one row of each against the live database, expecting no `notifications_type_check` violation. It reads the list from the source rather than keeping its own copy, because a hand-maintained second list is the same bug one level up — and it asserts the extracted list is non-trivially long, so a regex that stops matching fails loudly instead of passing vacuously.
> - **Build the new list from the previous migration's text, not by hand.** The `042 → 044` copy looked right on inspection; it was 22 near-identical single-quoted strings and the missing one was in the middle. Same family as the 033/036 replay failure above: the risk in these files is never the DDL, it is the list.
>
> **Editing an applied migration is still forbidden, and this was not an exception to that.** `044` had been applied to the local `_test` database only — never dev, never Render — so the fix was to correct the file, delete its single `schema_migrations` row, and re-run. Shipping a broken `044` plus a `045` fixing it would have been worse: two migrations for one change, and a window where any database on `044` alone is missing a notification type. **The rule holds the moment a file reaches any database someone else runs**; a local `_test`-only application is recoverable by hand and should be recovered that way.
...
> 🔐 **Migrating production from a laptop needs `DB_SSL=true`, or it dies with `SSL/TLS required`.** `config/db.js` turns SSL on only when `NODE_ENV === "production"` **or** `DB_SSL === "true"` — neither of which holds in an ordinary local shell, so the pool offers a plaintext connection and Render refuses it. Set `DB_SSL` alongside `DATABASE_URL` on the same command. Two dead ends worth not retrying: `NODE_ENV=production` also enables SSL but is a blunt switch that changes bcrypt cost, error verbosity and how `looksLikeProduction()` reads; and appending `?sslmode=require` to the connection string does nothing, because `poolConfig` passes `ssl` explicitly and that wins over the URL.
...
> - **`needsBootstrap()` in the client service never rejects** — an unreachable API or unmigrated database answers `false`, so a network blip shows the ordinary login form. Failing the other way would show a stranger a registration form.
> - **The endpoint stays public deliberately**: the caller has no session and there is nobody to authenticate as. It leaks one bit, only ever `true` for a database with no accounts, and `registerHrRoot` still demands `HR_REGISTRATION_CODE` — so the answer grants nothing that POSTing and reading the `409` wouldn't. A test pins that the body carries nothing else.

> 🧭 **The first invite on a brand-new organisation must be an `HR_ADMIN`, and the invite form now says so instead of dead-ending.** `ALLOWED_MANAGER_ROLES` lets `SUPER_ADMIN` manage an `HR_ADMIN` and nobody else, while `EMPLOYEE` needs a `MANAGER` or `HR_ADMIN` and `MANAGER` needs an `HR_ADMIN` — so on a freshly set-up org, picking any role other than HR admin left `ManagerSelect` rendering an empty `<select>` with no explanation, which reads as a broken form rather than as "invite an HR admin first". `ManagerSelect` now disables the picker and swaps its helper text when `options` is empty (and `allowNone` is false — with "No manager" a legitimate choice, an empty list is not an error state), and `InviteEmployeeForm` disables submit for the same condition. **Fixed by guiding, not by relaxing the reporting rules** — letting `EMPLOYEE`/`MANAGER` report directly to `SUPER_ADMIN` would change a deliberate decision (the root sits outside the tree) and ripple into three client copies of that map plus every scope helper.
...
> 🚦 **Rate limiting exists now, on the pre-auth routes only — `middlewares/rateLimiter.js` (`express-rate-limit`), closing what was the security doc's one HIGH finding.** bcrypt and `timingSafeEqual` made each individual guess expensive and uninformative; nothing capped *volume*, so login, Google login, `register/hr`, password reset and the single-use-token routes accepted unlimited attempts from one source. Five things about the design are load-bearing:
> - **Login counts failed attempts only** (`skipSuccessfulRequests`). The key is an IP and an office behind one NAT is a single IP, so counting successes would throttle a whole company signing in on a Monday morning. Brute force is all failures, so the ceiling only ever binds on the attack. **Don't "tighten" this by counting successes.**
> - **Password reset counts successes too**, and that asymmetry is deliberate: there a *success* is what sends an email, so the abuse being capped is mail-quota burn across many addresses — which `issuePasswordReset`'s cooldown cannot see, being keyed on `user_id`. The limit is on the IP, never the address, so it does not reintroduce the enumeration oracle that endpoint's three other properties exist to prevent.
> - **Deliberately not applied globally.** A limiter over the authenticated API would throttle ordinary use: the bell polls every 30s, the approvals badge every 30s, on top of page loads, all sharing one office IP. The pre-auth routes are the only ones where a high rate is never legitimate. Don't add a blanket `app.use(limiter)`.
> - **Off under `NODE_ENV=test` unless `RATE_LIMIT_ENABLED=true`**, and the flag is read **per request**, not captured at import (same convention as `mailFeatures.js`, same reason). The suite makes hundreds of logins from one loopback address; a limiter counting them would fail tests that have nothing to do with limiting. `rateLimiting.test.js` flips the flag on and calls `resetRateLimiters()` around each case — a limiter is stateful, so a leaked count is a failure in a later, unrelated test. That reset helper is the only reason each limiter is handed an explicit `MemoryStore` (which is what it would have built anyway).
> - **The 429 goes through `sendError`, never `next(err)`** — same `{ success, message, errors }` envelope as everything else, so the client needs no special case, and a rate-limited response can't be mistaken for an application error in the logs.
>
> Known and accepted: the store is in-memory, so counts are per process and reset on deploy. Right for the single Render web service; a second instance would need a shared (Redis) store before the numbers mean anything. `app.set("trust proxy", 1)` was already set for the cookie and is what makes the key the real client IP behind Render's proxy rather than the proxy's own — **if that line is ever removed, every visitor becomes one key and the limits become a company-wide outage.**
